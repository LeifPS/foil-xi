...
// der Move korrekt ausgelöst wurde. Fix: (1) die Icons werden schon beim Booten vorgeladen, nicht erst
// beim ersten Bedarf, (2) skDrawPlayer zeigt so oder so IMMER ein sichtbares Feedback (echtes Icon,
// sobald es fertig geladen ist, sonst ein Text-Fallback), nie mehr "gar nichts".
const { withPage } = require('./lib/browser');
const { ok, eq, noErrors, summary } = require('./lib/assert');

(async () => {
  const { result, errors } = await withPage(async (page) => page.evaluate(async () => {
    // (1) Vorladen beim Booten: der Icon-Cache darf nicht erst beim ersten Matchstart entstehen -
    // skMoveIconImgs() wird direkt nach der PLAYSTYLE_INFO-Definition aufgerufen (siehe index.html).
    const preloadedBeforeAnyMatch = Object.keys(skMoveIconImgs()).length >= 8
    ...
    return {preloadedBeforeAnyMatch, errorWhileNotLoaded, errorAfterPermanentFailure, errorWithNoIcon};
  }));

  console.log('Skill-Duell-Move-Icon-Fallback-Test');
  noErrors(errors, 'Seite');
  eq(result.preloadedBeforeAnyMatch, true, 'alle Move-Icons werden schon beim Booten vorgeladen, nicht erst beim ersten Matchstart');
